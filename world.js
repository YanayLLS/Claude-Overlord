...
function renderSel() {
  ...
  if (selected.unit) {
    ...
    if (u.mini) h = `<div class="w-portrait" style="--c:${hexStr(spec.color)}">S</div><div class="w-main" style="--c:${hexStr(spec.color)};--sc:var(--w-working)"><div class="w-crumb">${crumb} &middot; crew of ${esc(u.lead.a.title)}</div><h2>${esc(a.title)}</h2><div class="w-act">⚒ ${esc(a.verb || 'Working')}</div></div><div class="w-orders">${ob('&#x25A3;', 'Terminal', 'crewTerm')}</div>`;
      ...
      <div class="w-main" style="--c:${hexStr(spec.color)};--sc:var(${st.css})"><div class="w-crumb">${crumb}${a.team ? ' &middot; leads ' + a.team.members.length + (a.team.total ? ' (' + a.team.done + '/' + a.team.total + ' tasks)' : '') : ''}</div><h2>${esc(a.title)}</h2><div class="w-act">${st.sym} ${esc(activity(a))}${a.looping ? ' · looping' : ''}</div></div>
...
W.hoverRaid = function (id) { if (!alive) return; const rd = id == null ? null : raiders.get(String(id)) || null; if (rd === hoveredRaid) return; if (hoveredRaid && hoveredRaid.el) hoveredRaid.el.el.classList.remove('hov'); hoveredRaid = rd; if (rd) rd.el.el.classList.add('hov'); };
W.focusKey = function (kind, key) { if (!alive) return; if (kind === 'site') { const st = sites.get(key); if (st) { cam.userMoved = true; flyTo(st.g); select({ site: st }); } return; } if (kind === 'raid') { const rd = raiders.get(String(key)); if (rd) { cam.userMoved = true; const p = new THREE.Vector3(); rd.g.getWorldPosition(p); const fx = cam.target.x, fz = cam.target.z, fzoom = cam.zoom, tz = Math.min(cam.zoom, 1.15); tween(700, k => { cam.target.x = fx + (p.x - fx) * k; cam.target.z = fz + (p.z - fz) * k; cam.zoom = fzoom + (tz - fzoom) * k; }); select({ raid: rd }); } return; } if (kind === 'treasury') { const st = sites.get('treasury'); if (st) { cam.userMoved = true; flyTo(st.g); select({ site: st }); } return; } const o = kind === 'pr' ? ships.get(key) : kind === 'run' ? machines.get(key) : kind === 'peer' ? tents.get(key) : null; if (o) { cam.userMoved = true; flyTo(o.g); select(kind === 'pr' ? { pr: o.pr } : kind === 'run' ? { run: o.run } : { peer: o.peer }); } };
